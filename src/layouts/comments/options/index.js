...
export default function Options({ phoneNumber,lName,fName, userId, status }) {
  ...
  const handleClick = (event, type, id) => {
    ...
    if (type === "edit") {
      navigate(`/comments/EditComment/${id}`);}
    else if(type ==="detail")  {
      navigate(`/comments/commentsDetails/${id}`);
  ...
  const submit = () => {
    fetchApi(disableOrganUrl, { id, status: !status}, "put").then((res) => {
      if (res?.status_code === 200) {
        if (status) {
          toast.success(" Comment deactivated successfully! ");
        } else {
          toast.success(" Comment activated successfully! ");
        }
